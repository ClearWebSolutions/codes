...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function add($request){
		...
		if($request['gallery']==1){
			...
			//this goes before "use this->site->db_name" as it ends with "use codes"...
...
//		}
		...
		file_put_contents($this->site->dir."/class/".$this->classname.".php", "<?\nclass ".$this->classname." extends ".$this->classname."Base{\n\n}\n?>");
		file_put_contents($this->site->dir."/class/base/".$this->classname."Base.php", $this->_createClass($request));
		...
		if(!file_exists($this->site->dir."/admin/co.php")){
			dir_copy("modules/complexObject/admin/",$this->site->dir."/admin/");
		}
		if($request['categories']==1){
			dir_copy("modules/complexObject/templates/",$this->site->dir."/templates/");
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function update($request){
		...
		if($request['gallery']==1){
			...
			if($this->request['gallery']==1){
				...
				for($j=1;$j<=sizeof($this->prev_mod_gallery->gid);$j++){
					...
					$db->query("delete from ".$this->site->db_prefix."gallery2object where gallery_id='".$this->prev_mod_gallery->gid[$j]."' and object_table='".$this->tbl."'");
					//delete the old gallery from galleries
					$db->query("delete from ".$this->site->db_prefix."galleries where id='".$this->prev_mod_gallery->gid[$j]."'");
					//delete the old images
					$db->query("delete from ".$this->site->db_prefix."images where id='".$this->prev_mod_gallery->gid[$j]."'");
				}
			}
		}
		...
		//create the db table for future objects
		$db->query("DROP TABLE IF EXISTS ".$this->site->db_prefix.$this->tbl);
		$sql = $this->_createTableSQL($request);
		$db->query($sql);

		//create the class for objects
		if($this->classname!=$classname){
			@unlink($this->site->dir."/class/".$this->classname.".php");
			@unlink($this->site->dir."/class/base/".$this->classname."Base.php");
			file_put_contents($this->site->dir."/class/".$classname.".php", "<?\nclass ".$classname." extends ".$classname."Base{\n\n}\n?>");
		}
		file_put_contents($this->site->dir."/class/base/".$classname."Base.php", $this->_createClass($request));
		...
		if($this->classname!=$classname){
			...
			$replace[0] = "";
			$replace[1] = "";
			file_replace($this->site->dir."/class/includes/classes.php", $search, $replace);
			$classes = file($this->site->dir.'/class/includes/classes.php');
		...
		if($tbl!=$this->tbl){
			@unlink($this->site->dir."/admin/templates/".$this->tbl."_action.tpl");
			@unlink($this->site->dir."/admin/templates/".$this->tbl.".tpl");
		}
		//create the admin add/edit .tpl
		$this->_createAdminAddTpl($request);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function check($request){
		...
		if(($this->tbl!=$tbl&&$request['action']=='edit')||($request['action']=='add')){
			$db->query("use ".$this->site->db_name);
			$q = $db->query("show tables like '".$this->site->db_prefix.$tbl."'");
		...
		if($this->classname!=$classname){
			if(file_exists($this->site->dir."/class/".$classname.".php")){$this->error = "File /class/".$classname.".php already exists, please choose another DB table name as it's also used as a name for the class."; return false;}
			if(file_exists($this->site->dir."/class/base/".$classname."Base.php")){$this->error = "File /class/base/".$classname."Base.php already exists, please choose another DB table name as it's also used as a name for the class."; return false;}
		}
		if($this->tbl!=$tbl){
			if(file_exists($this->site->dir."/admin/templates/".$tbl.".tpl")){$this->error = "File /admin/templates/".$tbl.".tpl already exists, please choose another DB table name as it's also used as a name for the template."; return false;}
			if(file_exists($this->site->dir."/admin/templates/".$tbl."_action.tpl")){$this->error = "File /admin/templates/".$tbl."_action.tpl already exists, please choose another DB table name as it's also used as a name for the template."; return false;}
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function _createTableSQL($request){
		$tbl = normalize($request['db_tbl']);
		$sql = "CREATE TABLE ".$this->site->db_prefix.$tbl." (";
		...
		if($request['categories']==1){
			for($i=1;$i<=$request['categoriesttl'];$i++){
				if($request['category'.$i]&&$request['category'.$i.'_required']==1){
					foreach($this->site->categories as $c){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function _createAdminAddTpl($request){
		...
		file_put_contents($this->site->dir."/admin/templates/".$tbl."_action.tpl", $code);
	}
...
//TODO do the proper search for categories
		...
		file_put_contents($this->site->dir."/admin/templates/".$tbl.".tpl", $code);
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function _createBubbleCode4Page($request){
		...
		if($request['action']=='edit'){
			...
			file_replace($this->site->dir."/".$this->page->filename, $old_code, $replace);
		}else{
			//new code
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function _createBubbleCodeTpl($request){
		...
		if($tbl!=$this->tbl){
			...
			if(file_exists($this->site->dir."/templates/codes/co_".$this->tbl.".tpl")){
				@unlink($this->site->dir."/templates/codes/co_".$this->tbl.".tpl");
			}
		}
		...
		if($request['categories']==1){
			...
			for($i=1;$i<=$request['categoriesttl'];$i++){
				foreach($this->site->categories as $c){if($request['category'.$i]==$c['table']){$title = $c['title'];}}
...
$bubblecode .= "{/if}\n\n\t\t";
		...
		$bubblecode .= "</div>\n";
		$bubblecode .= "</div>";
		file_put_contents($this->site->dir."/templates/codes/co_".$tbl.".tpl", $bubblecode);
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function _createBubbleCodeDetailsTpl($request){
		...
		if($tbl!=$this->tbl){
			//delete the old bubble template
			if(file_exists($this->site->dir."/templates/codes/co_".$this->tbl."_details.tpl")){
				@unlink($this->site->dir."/templates/codes/co_".$this->tbl."_details.tpl");
			}
		}
		...
		file_put_contents($this->site->dir."/templates/codes/co_".$tbl."_details.tpl", $code);
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function _addBubbleCode2PageTpl($request){
		...
		$code_used = false;
		$code1 = "\n<div class=\"codes\">\n\t{if \$page=='".$this->page->name."'}{include file='codes/co_".$tbl.".tpl'}{/if}\n</div>\n\n";
		$new_template_code = "";
