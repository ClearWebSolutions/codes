...
//		$this->site->modules = array();
//		for($i=0;$i<sizeof($site->modules);$i++){
//			$this->site->modules[$i] = array("module"=>$site->modules[$i]['module']);
//			$this->site->modules[$i]['m'] = new stdClass();
//			$this->site->modules[$i]['m']->tbl = $site->modules[$i]['m']->tbl;
//		}
...
//		$request['products'] = "Products";
		...
		$site = new Site($_SESSION['siteid']);
		for($i=0;$i<sizeof($site->modules);$i++){
			if($site->modules[$i]['module']=='complexObject'&&$site->modules[$i]['m']->tbl==$request['products']){
				$this->products_module = $site->modules[$i]['m'];
				break;
			}
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function includeAdminStockManagement(){
		...
		if(!strstr($old, "stock_management.tpl")){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function update($request){
		...
		//do the options check
		if(!$this->checkStockBase($request)){return false;}

		//initializing page where the code would be updated
		$this->page = new Page($request['page']);

		//include the options to stock table, in case they changed 
		$this->updateStockTable2($request);

		//getting the complex object module selected
		$p_m = $this->products_module;//backing it up to use below
		$site = new Site($_SESSION['siteid']);
		for($i=0;$i<sizeof($site->modules);$i++){
			if($site->modules[$i]['module']=='complexObject'&&$site->modules[$i]['m']->tbl==$request['products']){
				$this->products_module = $site->modules[$i]['m'];
		...
		//insert all the required stuff to templates and other files of FRONTEND
				...
				//class/base/StockBase.php (options adjustments)
				copy("modules/store/copy/class/base/StockBase.php",$this->site->dir."/class/base/StockBase.php");
				$this->updateStockBase($request);

				//class/base/ShoppingCartBase.php (options adjustments)
				copy("modules/store/copy/class/base/ShoppingCartBase.php",$this->site->dir."/class/base/ShoppingCartBase.php");
				$this->updateShoppingCartBase();

				//class/base/OrderBase.php (update of the products class name)
				copy("modules/store/copy/class/base/OrderBase.php",$this->site->dir."/class/base/OrderBase.php");
				$this->updateOrderBase();

				//store.api.php update
				if($this->checkout != $request['checkout']){
					$search[0] = $this->checkout;
					$replace[0] = $request['checkout'];
					$this->checkout = $request['checkout'];
					file_replace($this->site->dir."/store.api.php", $search, $replace);
				}
				...
				//class/Products.php add $this->stock to constructor. Products.php is not a real name of the class the real name could be obtained from $request['products'] complex object
				//IMPORTANT we are adding to Products.php not to ProductsBase.php to not mess the ComplexObject in case it would be updated in future
				if($request['products']!=$this->request['products']){//do the update only if the product CO was changed
					$this->updateProducts($request);
				}

				//templates/codes/store_add2cart.tpl options adjustment
				copy("modules/store/copy/templates/codes/store_add2cart.tpl",$this->site->dir."/templates/codes/store_add2cart.tpl");
				$this->updateAdd2CartTpl($request);

				//templates/codes/co_product.tpl and templates/codes/co_product_details.tpl need to have the include of the store_add2cart.tpl
				if($this->request['products']!=$request['products']){
					$this->updateProductsBubble($request);
				}

				//templates/codes/store_review_order.tpl adjustment of the products_id param..., same for store_shopping_cart.tpl
				copy("modules/store/copy/templates/codes/store_review_order.tpl",$this->site->dir."/templates/codes/store_review_order.tpl");
				copy("modules/store/copy/templates/codes/store_shopping_cart.tpl",$this->site->dir."/templates/codes/store_shopping_cart.tpl");
				$this->updateProductIDs($request);

				//templates/checkout.tpl insertion of checkout bubble
				$str = file_get_contents($this->site->dir."/templates/header.tpl");
				if(!strstr($str, "store_checkout.tpl")){
					$this->insertCheckoutBubble($request);
				}

				//templates/header.tpl insertion of the shopping cart bubble
				$str = file_get_contents($this->site->dir."/templates/header.tpl");
				if(!strstr($str, "store_shopping_cart.tpl")){
					$this->updateHeaderTpl();
				}
		...
		//insert all the required stuff to templates and other files of ADMIN AREA
				...
				//update orders action link to the new one in case the CO products been changed
				if($request['products']!=$this->request['products']){//do the update only if the product CO was changed
					$search[0] = "co.php?classname=".$p_m->classname;
					$replace[0] = "co.php?classname=".$this->products_module->classname;
					file_replace($this->site->dir."/admin/templates/orders_action.tpl", $search, $replace);
				}
				...
				if($request['products']!=$this->request['products']){//do the update only if the product CO was changed
					$classname = $this->products_module->classname;
					$tpl = file_get_contents($this->site->dir."/admin/templates/orders_action.tpl");
					$new_tpl = str_replace("co.php?classname=Products", "co.php?classname=".$p_m->classname, $tpl);
					file_put_contents($this->site->dir."/admin/templates/orders_action.tpl", $new_tpl);
				}
				...
				//admin/templates/stock_management.tpl options!
				copy("modules/store/copy/admin/templates/stock_management.tpl",$this->site->dir."/admin/templates/stock_management.tpl");
				$this->updateAdminStockManagementOptions($request);
		...
		if($db->query("update modules set title='".addslashes($this->title)."', serialized='".base64_encode(serialize($this))."' where id='".$this->id."'")){
			...
			$this->error = "Can not update module in codes DB.";return false;
		}
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function updateStockTable2($request){
		global $db;
		//1: remove old options from stock table
		$db->query("use ".$this->site->db_name);
		for($i=1;$i<=$this->request['ttl'];$i++){
			if($this->request['title'.$i]){
				$sql = " DROP ".normalize($this->request['title'.$i]);
				@$db->query("ALTER TABLE ".$this->site->db_prefix."stock ".$sql);
			}
		}
		//2: add new options
