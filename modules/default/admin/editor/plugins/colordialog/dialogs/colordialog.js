...
CKEDITOR.dialog.add( 'colordialog', function( editor )
{
	var dialog, picker, pickerContents,
		// Actual el color value.
		elColor = document.getElementsByClassName('cke_button_textcolor')[0].getAttribute('selectedColor'),
//		elColor = editor.getUiColor(),
		pickerId = 'cke_uicolor_picker' + CKEDITOR.tools.getNextNumber();

	function setNewPickerColor( color )
	{
		//if our color is not in hex and not in yui rgb but just a calculated style rgb format which looks like rgb(0,0,0) string
		if ( /(.*?)rgb/.test(color) ){
	...
	function setNewColor( color, force )
	{
...
//		);
	...
	pickerContents =
	{
		id : 'yuiColorPicker',
		type : 'html',
		html : "<div id='" + pickerId + "' class='cke_uicolor_picker' style='width: 330px; height: 180px; position: relative;'></div>",
		onLoad : function( event )
		{
			var url = CKEDITOR.getUrl(
					'_source/' + // @Packager.RemoveLine
					'plugins/colordialog/yui/'
				);
			...
			// Set actual element color to the picker.
			if ( elColor )
				setNewPickerColor( elColor );
	...
	return {
		title : 'Select color',//editor.lang.uicolor.title,
		minWidth : 340,
		minHeight : 200,
		onLoad : function()
		{
...
/*								{
									id : 'livePeview',
									type : 'checkbox',
									label : 'Live preview',//editor.lang.uicolor.preview,
									'default' : 1,
									onLoad : function()
									{
...
/*								{
									...
									[
										{
											...
											type : 'select',
											'default' : '',
											label : 'Predefined color sets',//editor.lang.uicolor.predefined,
											items :
											[
												[ '' ],
												[ 'Light blue', '#9AB8F3' ],
								...
								/*{
									id : 'configBox',
									type : 'text',
									label : 'Paste this string into your config.js file',//editor.lang.uicolor.config,
									onShow : function()
									{
										var color = editor.getUiColor();
										if ( color )
