...
//AY removing the unnecessary footer
							...
							'</tbody></table>' +
//							'</tbody></table><div id="cke_bottom" role="presentation"',name,' class="cke_bottom">',bottomHtml,'</div>' +
...
//AY we've removed the footer, so the below thing doesn't exist anymore
		...
		buildDialog : function( editor )
		{
			...
			var element = CKEDITOR.dom.element.createFromHtml( [
					'<div class="', editor.id, '_dialog cke_editor_', editor.name.replace('.', '\\.'), '_dialog cke_skin_', editor.skinName,
						...
						//Hide the container when loading skins, later restored by skin css.
//AY bullshit this causes bugs in webkit by expanding the editor each time this style gets added
...
CKEDITOR.editor.prototype.resize = function( width, height, isContentHeight, resizeInner )
{
	...
	//AY
	if(width){	outer.setSize( 'width',  width, true ); }
